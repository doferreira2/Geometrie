...
void writeOFFfromValueMap(const Polyhedron& mesh, const Facet_double_map& facetMap, Facet_int_map& facetIntM , std::string filePath)
{
	...
	for (Facet_iterator i = mesh.facets_begin(); i != mesh.facets_end(); ++i)
	{
		...
		auto redValue = 1.0 -facetIntM.at(i); // low values will be closer to red
		auto greenValue = 0.0 ; // high values will be closer to green
		auto blueValue = 0.0 + facetIntM.at(i);
